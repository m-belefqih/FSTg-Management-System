...
$(document).ready(function(){
    // Check and render the ApexCharts area for students
    if($('#apexcharts-area').length > 0){
        var options = {
            ...
            xaxis: {
                categories: [
                    `${month}-${day-6}-${year}`, `${month}-${day-5}-${year}`, `${month}-${day-4}-${year}`,
                    `${month}-${day-3}-${year}`, `${month}-${day-2}-${year}`, `${month}-${day-1}-${year}`, 
                    `${month}-${day}-${year}`
                ]
            }
        };
    ...
    // Check and render the school-area for students
